...
def show_tags(context):
    tag_list = (
        Tag.objects.annotate(num_posts=Count("post"))
    ...
    try:
        ...
        for tag in tag_list:
            tag.color_level = (
                int((tag.num_posts - minNum) / (maxNum - minNum) * (levelNum - 1)) + 1
            )
...
@register.simple_tag(takes_context=False)
def toc_not_null(toc: str):
    if toc.replace(" ", "").replace(
        "\n", ""
    ) == '<div class="toc"><ul></ul></div>'.replace(" ", "").replace("\n", ""):
        return False
    else:
        return True
