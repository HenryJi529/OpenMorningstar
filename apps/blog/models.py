...
class Post(models.Model):
    ...
    readtime = models.IntegerField("阅读时间", default=0, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        self.readtime = (
            self.readtime
            if self.readtime
            else int(readtime.of_markdown(self.body, wpm=200).seconds / 60)
        )  # 设置默认值
...
class Comment(models.Model):
    user = models.ForeignKey(
        User, verbose_name="用户", on_delete=models.SET_NULL, null=True
    )
    ...
    thumbs_up = models.ManyToManyField(
        User, related_name="thumbsUp_user", verbose_name="点赞"
    )
    thumbs_down = models.ManyToManyField(
        User, related_name="thumbsDown_user", verbose_name="点踩"
    )
