...
def updatePassword(request):
    ...
    if update_password_form.is_valid():
        ...
        error_message = [
            value[0]["message"]
            for value in json.loads(update_password_form.errors.as_json()).values()
        ][0]
...
def updateEmail(request):
    ...
    if update_email_form.is_valid():
        ...
        messages.add_message(
            request, messages.ERROR, update_email_form.errors["email_code"][0]
        )
    return redirect(reverse("blog:index"))
...
def updatePhone(request):
    ...
    if update_phone_form.is_valid():
        ...
        messages.add_message(
            request, messages.ERROR, update_phone_form.errors["phone_code"][0]
        )
    return redirect(reverse("blog:index"))
