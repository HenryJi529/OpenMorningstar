(function (doc, script, anchor, target, style) {
	// get currently executing script
	script = doc.currentScript || doc.scripts[doc.scripts.length - 1];

	/**
	 * Gets value from script dataset.
	 *
	 * @param  {String} key
	 * @return {String}
	 */
	function getData(key) {
		return (
			script.dataset ? script.dataset[key] : script.getAttribute('data-' + key)
		);
	}
	...
	anchor.href = getData('href')
	anchor.className = 'github-corner';
	anchor.setAttribute('aria-label', getData('label') || 'View source on Github');
	...
	// append respective elements to head and body
