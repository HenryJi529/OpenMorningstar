...
class LoginForm(forms.Form):
    ...
    password = forms.CharField(
        ...
        widget=forms.PasswordInput(
            attrs={"placeholder": "********", "class": "w-full"}, render_value=True
        ),
        required=True,
    )
    image_captcha = forms.CharField(
        label="人机验证",
        required=True,
        widget=forms.TextInput(
            attrs={"placeholder": "XXXXXX", "class": "w-full"},
        ),
    )
...
class RegisterForm(forms.Form):
    ...
    password = forms.CharField(
        ...
        required=True,
        widget=forms.PasswordInput(
            attrs={"placeholder": FAKE_PASSWORD, "class": "w-full"}
        ),
        validators=[
            RegexValidator(r"^[\w_]*$", "密码中只能包含数字、字母下划线"),
        ],
        min_length=6,
        max_length=16,
        error_messages={
...
class InfoForm(forms.Form):
    nickname = forms.CharField(
        label="昵称", widget=forms.TextInput(attrs={"class": "w-full"})
    )
    bio = forms.CharField(
        label="个人介绍",
        widget=forms.Textarea(attrs={"class": "textarea w-full", "rows": 4}),
    )
    avatar = forms.ImageField(
        label="头像",
        required=False,
        widget=forms.FileInput(
            attrs={"class": "absolute cursor-pointer w-full", "style": "opacity: 0;"}
        ),
    )
