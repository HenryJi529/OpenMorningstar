...
def handle_register(request: HttpRequest, is_api=True):
    ...
    if form.is_valid():
        ...
        subject = "激活邮件"
        message = create_activate_message(
            username=username, host=request.META["HTTP_HOST"]
        )
...
def handle_login(request, is_api=False):
    ...
    if form.is_valid():
        ...
        if is_api:
            return JsonResponse(
                {"status": "error", "message": form.errors["image_captcha"][0]}
            )
        else:
            messages.add_message(
                request, messages.ERROR, form.errors["image_captcha"][0]
            )
            return redirect("/")
